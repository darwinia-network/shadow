...
// Pack encode proof
func encodeProofArray(arr []internal.DoubleNodeWithMerkleProof) string {
	hex := "0x0101"
	for _, v := range arr {
		hex += encodeProof(v)
	}
...
func encodeProof(dnmp internal.DoubleNodeWithMerkleProof) string {
	hex := ""
...
func encodeDarwiniaEthHeader(header internal.DarwiniaEthHeader) string {
	hex := "0x"
